...
class Display:
    ...
    def display_recommendation(self, recommendations):
        ...
        if recommendations != None:
                ...
                with column:
                    for recipe in recommendations[rows*row:rows*(row+1)]:
                        ...
                        nutritions_df = pd.DataFrame(
                            {value: [recipe[value]] for value in nutrition_values})
            ...
            st.info(
                'Couldn\'t find any recipes with the specified ingredients', icon="🙁")
...
with st.form("recommendation_form"):
    ...
    nutritions_values_list = [Calories, FatContent, SaturatedFatContent, CholesterolContent,
                              SodiumContent, CarbohydrateContent, FiberContent, SugarContent, ProteinContent]
...
if generated:
    with st.spinner('Generating recommendations...'):
        recommendation = Recommendation(
            nutritions_values_list, nb_recommendations, ingredient_txt)
